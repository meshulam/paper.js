...
import {
    getValues, classify, subdivide, evaluateCurve, getLength,
    isStraight, isStraightInternal, getTimeAt,
} from './CurveUtils';
...
export const Curve = Base.extend(/** @lends Curve# */{
    ...
    getValues: function(matrix) {
        return getValues(this._segment1, this._segment2, matrix);
    },
    ...
    getLength: function() {
        if (this._length == null)
            this._length = getLength(this.getValues(), 0, 1);
        return this._length;
    },
    ...
    getPartLength: function(from, to) {
        return getLength(this.getValues(), from, to);
    },
    ...
    divideAtTime: function(time, _setHandles) {
        ...
        if (time >= tMin && time <= tMax) {
            var parts = subdivide(this.getValues(), time),
                left = parts[0],
                right = parts[1],
...
statics: /** @lends Curve */{
    ...
    getPart: function(v, from, to) {
        ...
        if (from > 0)
            v = subdivide(v, from)[1]; // [1] right
        // Interpolate the parameter at 'to' in the new curve and cut there.
        if (to < 1)
            v = subdivide(v, (to - from) / (1 - from))[0]; // [0] left
...
/** @lends Curve# */{
    ...
    getTimeAt: function(offset, start) {
        return getTimeAt(this.getValues(), offset, start);
    },
...
new function() { // Scope for methods that require private functions
    return { statics: {
        getPoint: function(v, t) {
            return evaluateCurve(v, t, 0, false);
        },

        getTangent: function(v, t) {
            return evaluateCurve(v, t, 1, true);
        },

        getWeightedTangent: function(v, t) {
            return evaluateCurve(v, t, 1, false);
        },

        getNormal: function(v, t) {
            return evaluateCurve(v, t, 2, true);
        },

        getWeightedNormal: function(v, t) {
            return evaluateCurve(v, t, 2, false);
        },

        getCurvature: function(v, t) {
            return evaluateCurve(v, t, 3, false).x;
        },
    }};
},
    ...
    function getCurveIntersections(v1, v2, c1, c2, locations, include) {
        ...
        if (max(v1[0], v1[2], v1[4], v1[6]) + epsilon >
            ...
            if (overlaps) {
                ...
                var straight1 = isStraight(v1),
                    straight2 = isStraight(v2),
